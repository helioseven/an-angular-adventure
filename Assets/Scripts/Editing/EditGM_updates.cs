...
public partial class EditGM {
	...
	// updates getInputs and getInputDowns each frame
	private void updateInputs ()
	{
		InputKeys nowKeys = GetAllInputs();
		getInputs = nowKeys;
		getInputDowns = (nowKeys & getInputDowns);
	}

	// updates UI Overlay and Palette panels
	private void updateUI ()
	{
		bool sbCKD = CheckKeyDowns(InputKeys.HUD);
		bool tabCK = CheckKeys(InputKeys.Palette);
	...
	{
		if (CheckKeyDowns(InputKeys.ClickAlt)) anchorIcon.FindNewAnchor(); // <2>

		if (CheckKeyDowns(InputKeys.Out)) activateLayer(activeLayer - 1); // <3>
		if (CheckKeyDowns(InputKeys.In)) activateLayer(activeLayer + 1);
	...
	private void updateCreate ()
	{
		...
		if (b1) {
			...
			if (CheckKeyDowns(InputKeys.CCW)) tileCreator.SetRotation(rot + 1); // <2>
			if (CheckKeyDowns(InputKeys.CW)) tileCreator.SetRotation(rot - 1);

			if (CheckKeyDowns(InputKeys.ColorCCW)) tileCreator.CycleColor(false);
			if (CheckKeyDowns(InputKeys.ColorCW)) tileCreator.CycleColor(true);

			if (CheckKeyDowns(InputKeys.ClickMain)) addTile(); // <3>
		}
		...
		if (b2) {
			...
			if (CheckKeyDowns(InputKeys.ClickMain)) addSpecial(new ChkpntData(anchorIcon.focus, activeLayer));
		}
		if (b3) {
			...
			if (CheckKeyDowns(InputKeys.ClickMain)) addSpecial(wd);
		}

		if (!b2 && CheckKeyDowns(InputKeys.Chkpnt)) setTool(chkpntTool); // <5>
		if (!b3 && CheckKeyDowns(InputKeys.Warp)) setTool(warpTool);
		bool bType = false;
	...
	private void updateEdit ()
	{
		if (selected_item.HasValue) {
			...
			if (CheckKeyDowns(InputKeys.ClickMain)) {
		...
		} else if (CheckKeyDowns(InputKeys.ClickMain)) { // <5>
	...
	{
		if (CheckKeyDowns(InputKeys.ClickMain)) { // <1>
			Ray r = Camera.main.ScreenPointToRay(Input.mousePosition);
